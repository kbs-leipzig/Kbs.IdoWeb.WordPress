<?php /* Template Name: kbs_erkennen_general_v2 */ ?>
<?php get_header(); ?>
...
<header class="entry-header">
	...
	<style>
		...
		.no_more_ripping_icon {
			display: none;			
		}				
		
		.no_more_ripping_icon > i {
			margin-right: 10px;
		}
		
		#dkg_panelbar.k-panelbar > li.k-item.k-state-default.no_more_ripping .no_more_ripping_icon {
...
<div class="entry-content">
    ...
    <script type="text/javascript">
		...
		var DescriptionKeyModel = kendo.data.Node.define ({
			...
			fields: {
					...
					"ParentDescriptionKey": { type: "number", defaultValue: null },
					"DescriptionStep": { type: "string", defaultValue: null },
					"BadgeValue": { type: "number", defaultValue: 0 },
				    "IsVisible": { type: "number", defaultValue: 0 },
			},
			sort: [{field: "BadgeValue", dir: "desc"}],
			sortable: true,
			hasChildren: false
		  });
		 ...
		 var DKGModel = kendo.data.Node.define({
            ...
            fields: {
				...
				KeyGroupDescription: { type: "string", defaultValue: null },
        ...
        var keyGroup_DataSource_full = new kendo.data.HierarchicalDataSource({
            ...
            sort: [{field: "VisibilityCategoryId", dir: "asc"},{ field: "OrderPriority", dir: "asc" },{ field: "RipValue", dir: "asc" }],
	...
	function g_createFigCaptionDom(data, $imgDom) {
		...
		function g_getKeyGroupDescriptionByDKGId(dkgId) {
            if (keyGroup_DataSource_full) {
                if (typeof (keyGroup_DataSource_full.get(dkgId)) !== 'undefined') {
                    return keyGroup_DataSource_full.get(dkgId).get("KeyGroupDescription");
                }
            }
            return false;
        }
						...
						<script id="panelbar_dkg_template" type="text/kendo-ui-template">
							...
							# if (item.DescriptionKey) { #
								...
								#var dkgtype_classname = "default";#
									...
									# var ampel_wert = "";
									  if(item.VisibilityCategoryId == 1) {
									  	ampel_wert = "Tot, meist mit Präparation, mit min. 40x Auflichtmikroskop oder anderen Mikroskopiertechniken";
									  } 
									  else if(item.VisibilityCategoryId == 2) 
									  {
									  	ampel_wert = "Lebend/tot, ohne Präparation, mit 20x Auflichtmikroskop oder guter Makroaufname";
									  }
									  else if(item.VisibilityCategoryId == 3) 
									  {
									  	ampel_wert = "Lebend, ohne Präparation, mit Auge bis 10x Lupe oder Foto";
									  }
									  else {
									  	ampel_wert = "Keine Ampelfarbe";
									  } 
									  
								let kgDescription = g_getKeyGroupDescriptionByDKGId(item.DescriptionKeyGroupId);
								#
								...
								# } else { #
									<div class="pb-item-header-wrap">
										...
										<span class="dkg_info_wrap">
											<div class="no_more_ripping_icon"><i title="Diese Merkmale bieten keine weitere Auftrennung" class="fas fa-window-close"></i>&nbsp;|&nbsp;</div>
											# if (kgDescription != null) {#
												<i title="#=g_getKeyGroupDescriptionByDKGId(item.DescriptionKeyGroupId)#" class="fas fa-info"></i>&nbsp;|&nbsp;
											#}#
											
											<i title="#:ampel_wert#" class="ampel-#:item.VisibilityCategoryId#"></i>
											&nbsp;|&nbsp;<i title="Geschlecht" class="fas dkgtype-#:dkgtype_classname#"></i>
											&nbsp;|&nbsp;<i class="fas fa-arrows-alt" title="#=g_getLageRegionByParentDKGId(item.ParentDescriptionKeyGroupId)#"></i>
										</span>
									</div>
								# } #
							# } else { #
							...
							<div class="dk_value_wrapper">
								<span class="dk_list_item" data-badgevalue="#:item.BadgeValue#" data-badge="#item.BadgeValue#" data-isvisible="#:item.IsVisible#" data-desckeyid="#: item.DescriptionKeyId #">#: item.KeyName #</span>
							...
							<div data-type="#=dkType#" title="Merkmal auswählen" class="heading_flex_item">
								<span class="dk_list_item" data-desckeyid="#:item.DescriptionKeyId#" data-badge="#:item.BadgeValue#" data-isvisible="#:item.IsVisible#">#: item.KeyName #
						...
						<script>
							...
							var viewModel_dkg_pb = kendo.observable({
								...
								updateFilter: function () {
									...
									this.data.sort([{field: "VisibilityCategoryId", dir: "desc"},{ field: "OrderPriority", dir: "asc" },{ field: "RipValue", dir: "asc" },{field: "DescriptionKey.BadgeValue", dir: "asc"}]);
								...
								updateBadges: function () {
									...
									//console.log($(that.dk_list_item_selector).closest('li.k-item'));
									$(that.dk_list_item_selector).closest('li.k-item').addClass('k-state-disabled').addClass('k-state-hidden');
									$(that.dk_pic_list_item_selector).closest('li.k-item').addClass('k-state-disabled').addClass('k-state-hidden');
									var viewItems = this.data.view();
									viewItems.forEach(function (dKeyGroup) {										
										if(dKeyGroup && dKeyGroup.DescriptionKey) {
											dKeyGroup.DescriptionKey.forEach(function (dKey) {
											    dKey.set("BadgeValue", 0);
												dKey.set("IsVisible", 0);
												dKey.dirtyFields = {};
												dKey.dirty = false;
											});
										}
									...
									if (this.badgesList) {
										...
										console.log(this.badgesList);
											that.badgesList.forEach(function (key, val) {
												//$("#dkg_panelbar>li.k-item>ul.k-group>li.k-item span.dk_list_item").first("[data-desckeyid=68929]").closest("li.k-item");
												$dk_list_item = $(that.dk_list_item_selector).closest("[data-desckeyid=" + val + "]");
												$dk_pic_list_item = $(that.dk_pic_list_item_selector).closest("[data-desckeyid=" + val + "]");
												var dataItem = that.data.getByUid($dk_list_item.closest('li.k-item').data("uid"));
												if(typeof(dataItem) !== 'undefined') {
													dataItem.set("BadgeValue", key);
													dataItem.set("IsVisible", 1);
													$dk_list_item.closest('li.k-item').removeClass('k-state-hidden');
													$dk_pic_list_item.closest('li.k-item').removeClass('k-state-hidden');
												}
												if(key === 0) {
													$dk_list_item.closest('li.k-item').addClass('k-state-disabled');
													$dk_pic_list_item.closest('li.k-item').addClass('k-state-disabled');
												} else {
													//$("#dkg_panelbar>li.k-item>ul.k-group>li.k-item span.dk_list_item").closest("[data-desckeyid=68918]").closest("li.k-item").removeClass("k-state-disabled");
													//console.log("removing disabled");
													//console.log("---");													
													//console.log(dataItem.uid);
													//console.log($("[data-uid="+dataItem.uid+"]"));
													if(typeof(dataItem) !== 'undefined') {
														$("[data-uid="+dataItem.uid+"]").removeClass("k-state-disabled");
													}
													/*
													console.log($dk_list_item);
													console.log($dk_list_item.closest('li.k-item'));
													console.log($dk_list_item.closest('li.k-item').hasClass('k-state-disabled'));
													$dk_list_item.closest('li.k-item').removeClass('k-state-disabled');
													console.log($dk_list_item.closest('li.k-item').hasClass('k-state-disabled'));
													$dk_pic_list_item.closest('li.k-item').removeClass('k-state-disabled');
													*/
												}
										...
										that.setSort();
										$("[data-badge=0]").each(function() {
										  $(this).closest('li.k-item').addClass('k-state-disabled');
										});
										$("[data-isvisible=0]").each(function() {
										  $(this).closest('li.k-item').addClass('k-state-hidden');
										});
									...
									/*
									var v = $("#dkg_panelbar").data("kendoPanelBar").dataSource.view();
									v.forEach(function (item) {
										item.children.data().forEach(function (child) {
											child.index = 1000 - child.BadgeValue;
										})
										//console.log(item.children.sort([{field: "BadgeValue", dir: "desc"}]));
										//console.log(item.children.data());
										//item.children.data.sort([{field: "BadgeValue", dir: "desc"}]);
								...
								updateRipValues: function () {
									...
									if (this.ripOrder !== null) {
										...
										Object.keys(that.ripOrder).forEach(function (ripItem) {
											...
											if (typeof (dataItem) !== 'undefined') {
												dataItem.set("RipValue", that.ripOrder[ripItem]);
											}
										});
								...
								updateAllInfo: function () {
									console.log("updateAllInfo");
									this.updateBadges();
									this.disableZeroDks();
								},
								setSort: function (){
									var items = $("#dkg_panelbar").data("kendoPanelBar").dataSource.view();
									//console.log(items);
								...
								markNoMoreRippingDKGs: function () {
									console.log("markNoMoreRippingDKGs");
									$("#dkg_panelbar>li.k-item[role=menuitem]:not(.k-state-disabled)").each(function(dkgitem){
										$(this).removeClass("no_more_ripping");
										//console.log($(this).find(".dk_list_item[data-badge!=0]"));
										if($(this).find(".dk_list_item[data-badge!=0]").length < 2) {
											$(this).addClass("no_more_ripping");
										}
									});
								},								
